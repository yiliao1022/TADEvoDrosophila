...
system "bedtools intersect -a $ARGV[0].shuffle.$i.bed -b /data/users/liaoy12/liaoy12/TAD_SV/7_SV_TAD_2020Feb18/Boundaries/2NCpapes.overlap.tad.boundaries.uniq.bed.euchromatin.bed.euchromatin.bed.bed | awk '{sum+=(\$3-\$2)} END {print sum}' >> $ARGV[0].permutation.cov.bed";
