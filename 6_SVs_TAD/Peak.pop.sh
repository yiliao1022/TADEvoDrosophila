...
export DEL=pop.del.bed
export INS=pop.nonTE.ins.bed
export TE=pop.TE.ins.bed
export CNV=all.pop.CNV.derived.bed.bed.sort.bed.euchromatin.bed
export CNVALL=pop.cnv.bed
...
cat $DEL | awk '$3-$2<2001' | awk -F "\t" '{ if (gsub(/1/, "1", $4) >1 ) print $1"\t"$2"\t"$3}' > $DEL.uniq.bed_common
cat $DEL | awk '$3-$2<2001' | awk -F "\t" '{ if (gsub(/1/, "1", $4) ==1 ) print $1"\t"$2"\t"$3}' > $DEL.uniq.bed_rare
