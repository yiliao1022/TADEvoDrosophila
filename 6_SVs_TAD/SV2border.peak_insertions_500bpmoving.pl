...
for($i=-$m;$i<$m+1;$i++) {
 ...
 while (<In>) {
  ...
  system "bedtools intersect -wa -wb -a $ARGV[1] -b $ARGV[0].flank.$i\_K.bed -u | awk '{sum+=\$4} END {print sum}' >> $ARGV[1].peak.cov.bed";
...
system "paste coordinate.bed $ARGV[1].peak.cov.bed > $ARGV[1].peak.cov.bed.bed";
system "rm $ARGV[1].peak.breatpoints.bed $ARGV[1].peak.cov.bed";
